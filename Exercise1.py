...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description= "A program which decrypts texts crypted with poli-alphabetic method with known key length (using gen algos)")
    ...
    parser.add_argument('nind', help="Number of individuals per population (goes from 100 for key length 4 to 800 for key legnth 15)", type = int)
...
class Individual(list):
    """Ogni individuo è una chiave di cifratura della lunghezza specificata in input da riga di comando"""
...
def generate_population(nind, length):
    """Genera una popolazione iniziale di nind individui chiave di lunghezza length"""
...
if __name__ == '__main__':
    ...
    pop = generate_population(nind, key_length)
